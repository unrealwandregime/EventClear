...
MARKETS = [
    {
        "conditionId": "btc-100",
        "question": "Will BTC official index close above $100,000?",
        "asset": "BTC",
        "threshold": 100000,
        "observation": "CLOSE_AT",
        "endDate": "2026-12-31T23:59:59Z",
        "tokenIds": ["btc-100-y", "btc-100-n"],
        "active": True,
    },
    {
        "conditionId": "btc-150",
        "question": "Will BTC official index close above $150,000?",
        "asset": "BTC",
        "threshold": 150000,
        "observation": "CLOSE_AT",
        "endDate": "2026-12-31T23:59:59Z",
        "tokenIds": ["btc-150-y", "btc-150-n"],
        "active": True,
    },
    {
        "conditionId": "eth-any",
        "question": "Will ETH trade above $8,000 at any time?",
        "asset": "ETH",
        "threshold": 8000,
        "observation": "REACHES_ANY_TIME",
        "endDate": "2026-09-30T23:59:59Z",
        "tokenIds": ["eth-any-y", "eth-any-n"],
        "active": True,
    },
    {
        "conditionId": "eth-5000",
        "question": "Will ETH official index close above $5,000?",
        "asset": "ETH",
        "threshold": 5000,
        "observation": "CLOSE_AT",
        "endDate": "2026-09-30T23:59:59Z",
        "tokenIds": ["eth-5000-y", "eth-5000-n"],
        "active": True,
    },
    {
        "conditionId": "eth-8000",
        "question": "Will ETH official index close above $8,000?",
        "asset": "ETH",
        "threshold": 8000,
        "observation": "CLOSE_AT",
        "endDate": "2026-09-30T23:59:59Z",
        "tokenIds": ["eth-8000-y", "eth-8000-n"],
        "active": True,
    },
]
...
POSITIONS = [
    {
        ...
        "currentValueAtomic": str(64 * UNIT),
        "title": "BTC closes above $100,000 — YES",
    },
    {
        "conditionId": "0x" + "22" * 32,
        "tokenId": "4",
        "outcome": "NO",
        "amountAtomic": str(100 * UNIT),
        "currentValueAtomic": str(62_400_000),
        "title": "BTC closes above $150,000 — NO",
    },
    {
        "conditionId": "eth-5000",
        "tokenId": "eth-5000-y",
        ...
        "currentValueAtomic": str(44 * UNIT),
    },
    {
        "conditionId": "eth-8000",
        "tokenId": "eth-8000-n",
        "outcome": "NO",
        "amountAtomic": str(100 * UNIT),
        "currentValueAtomic": str(61 * UNIT),
    },
]
...
class MemoryStore:
    def __init__(self) -> None:
        ...
        self.bundles = [
            {
                ...
                "onchainBundleId": 418,
                "status": "ACTIVE",
                "principalAmountAtomic": str(100 * UNIT),
                "grossAdvanceAtomic": "95000000",
                "netAdvanceAtomic": "94525000",
                "conditionsResolved": False,
                "unresolvedConditions": ["0x" + "11" * 32, "0x" + "22" * 32],
                "relationshipDefinitionHash": HASH,
                "artifactHash": "0x" + "00" * 32,
            },
            {
                "id": "EC-00391",
                "status": "SETTLED",
                "principalAmountAtomic": str(80 * UNIT),
                "settlementProceedsAtomic": str(180 * UNIT),
            },
            {
                "id": "EC-00372",
                "status": "SHORTFALL",
                "principalAmountAtomic": str(100 * UNIT),
                "settlementProceedsAtomic": str(75 * UNIT),
                "shortfallAtomic": str(25 * UNIT),
            },
        ]
        self.claims = [
            {
                "tokenId": "principal-418",
                "bundleId": "EC-00418",
                "claimType": "PRINCIPAL",
                "supplyAtomic": str(100 * UNIT),
                "holderAddress": "0x0000000000000000000000000000000000000001",
                "holderBalanceAtomic": str(100 * UNIT),
            },
            {
                "tokenId": "residual-418",
                "bundleId": "EC-00418",
                "claimType": "RESIDUAL",
                "supplyAtomic": str(10**18),
                "holderAddress": "0x0000000000000000000000000000000000000001",
                "holderBalanceAtomic": str(10**18),
            },
        ]
        ...
        self.erc1155_approvals: dict[tuple[str, str], bool] = {}
        self.lp_allowlist: set[str] = {"0x0000000000000000000000000000000000000001"}
        self.next_nonce = 1
    ...
    def get_market(self, condition_id: str) -> dict | None:
        item = next(
            (item for item in self.markets if item["conditionId"] == condition_id), None
        )
    ...
    def get_relationship(self, relationship_id: str) -> dict | None:
        item = next(
            (item for item in self.relationships if item["id"] == relationship_id), None
        )
        return deepcopy(item) if item else None
    ...
    def get_relationship_by_hash(self, definition_hash: str) -> dict | None:
        item = next(
            (
                ...
                if item["canonicalDefinitionHash"] == definition_hash
            ),
            None,
        )
        return deepcopy(item) if item else None
    ...
    def get_idempotency(self, key: str) -> dict | None:
        value = self.idempotency_records.get(key)
        return deepcopy(value) if value else None

    def save_idempotency(self, key: str, value: dict) -> None:
        self.idempotency_records[key] = deepcopy(value)
